...
$update=date('d-m-Y h:i:s', time());
	...
	if($row_cnt>0)
	{
		$con="update userregistration set password=?,passUdateDate=?  where userid=?";
$chngpwd1 = $mysqli->prepare($con);
$chngpwd1->bind_param('ssi',$np,$update,$aid);
  $chngpwd1->execute();
...
$ret="select * from userregistration where userid=?";
	 ...
	 //$cnt=1;
	  ...
	  {
	  	?>									
					Last Updation Date:&nbsp;<?php echo $row->passUdateDate ; ?> </div>
	  <?php } ?>
